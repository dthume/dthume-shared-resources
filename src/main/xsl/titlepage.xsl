...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                xmlns:lsx="alias://www.w3.org/1999/XSL/Transform"
...
<xsl:template match="t:templates">
  <lsx:stylesheet xmlns:d="http://docbook.org/ns/docbook"
      xmlns:exsl="http://exslt.org/common"
      xmlns:fo="http://www.w3.org/1999/XSL/Format">
    <xsl:attribute name="version">1.0</xsl:attribute>
    <xsl:attribute name="exclude-result-prefixes">exsl d fo</xsl:attribute>
...
<xsl:template match="t:titlepage">
  ...
  <xsl:element name="xsl:template">
    ...
    <xsl:element name="{@t:wrapper}">
      ...
      <xsl:element name="xsl:if">
        ...
        <xsl:text>&#xA;      </xsl:text>
        <!-- Removed extra wrapper for recto / verso; html only uses recto -->
        <!-- <xsl:element name="{@t:wrapper}"> -->
  ...
  <xsl:apply-templates mode="output.titlepage.content.templates.mode"
    select="t:titlepage-content/*" />

</xsl:template>

<xsl:template match="@*" mode="output.titlepage.content.templates.mode"></xsl:template>

<xsl:template match="*" mode="output.titlepage.content.templates.mode">
  ...
  <!-- to be off, or has already been output, don't output it.      -->
    ...
    <xsl:if test="(not(@t:suppress-template) or @t:suppress-template='0')
      ...
      <xsl:variable name="wrapper">
        <xsl:choose>
          <xsl:when test="@t:wrapper">
            <xsl:value-of select="@t:wrapper" />
          </xsl:when>
          <xsl:when test="../../@t:wrapper">
            <xsl:value-of select="ancestor::t:titlepage/@t:wrapper" />
          </xsl:when>
        </xsl:choose>
      </xsl:variable>
      <xsl:text>&#xA;&#xA;</xsl:text>
...
<xsl:template match="t:group" mode="output.titlepage.content.templates.mode">
...
<xsl:template match="t:group" mode="stylesheet.order">
  <xsl:text>&#xA;  </xsl:text>
  <xsl:element name="{@t:wrapper}">
    <xsl:apply-templates mode="stylesheet.order" select="child::*" />
  </xsl:element>
</xsl:template>
